...
export default defineConfig({
  plugins: [
    react(),
    // Dev 模式下模拟 nginx 的 MPA 路由重写：
    // /platform/* → platform.html，其余 → index.html
    {
      name: 'mpa-dev-router',
      configureServer(server) {
        server.middlewares.use((req: Connect.IncomingMessage, _res, next) => {
          const url = req.url ?? '/'
          if (url.startsWith('/platform') && !url.startsWith('/platform.html') && !url.startsWith('/src/') && !url.startsWith('/node_modules/') && !url.startsWith('/@') && !url.startsWith('/api/')) {
