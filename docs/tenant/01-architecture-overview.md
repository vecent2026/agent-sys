...
| 概念 | 说明 |
|------|------|
| **平台（Platform）** | SaaS 运营方，通过平台用户+平台角色体系管理租户与全局权限；超管是 `platform_role.is_super=1` 的角色 |
...
平台用户 (platform_user)
  ...
  └── 权限: 通过平台角色判定；若用户持有 `is_super=1` 角色则授予全量平台权限，不受 tenant_id 过滤
...
4. **租户数据强隔离**：任何跨租户查询必须通过平台端受控接口（通常由具备相应平台权限的管理员访问），不允许租户侧绕过
