...
**preToken 使用限制**：
- 只能访问 `POST /api/tenant/auth/select` 或 `POST /api/tenant/auth/select-tenant`
...
if ("PRE_TOKEN".equals(tokenType)) {
    ...
    if (!"/api/tenant/auth/select".equals(requestUri)
            && !"/api/tenant/auth/select-tenant".equals(requestUri)) {
...
补充约束（租户内置超管角色）：
- 每个租户默认存在 `tenant_admin` 角色（`is_builtin=1,is_super=1`）
- 该角色默认拥有当前租户全部授权节点（由 `tenant_permission` 同步）
- 该角色不可编辑、不可删除（前后端均需保护）
...
补充约束（最后一名超管保护）：
- `PUT /api/tenant/members/{userId}/roles`：若目标成员是该租户最后一名超管，禁止移除其超管角色
- `DELETE /api/tenant/members/{userId}`：若目标成员是该租户最后一名超管，禁止删除
- 后端返回明确错误原因，前端按 message 原样提示用户
...
| 黑名单命中（登出/强制失效） | Token 已失效 |
| 租户 token 缺失 tenantId | 租户上下文缺失 |
| Token 版本字段解析失败 | Token 版本格式错误 |
