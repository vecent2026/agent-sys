...
表单字段：
  ...
  ── 初始管理员 ──
  - 管理员手机号（选填；填写后必须同时设置初始管理员密码）
  - 初始管理员密码（填写管理员手机号时必填，长度 6-20 位）
  - 管理员昵称（必填）
...
## 10. 2026-03-18 前端交互收敛补充

### 10.1 权限树勾选交互统一

平台端与租户端的权限树必须复用同一套父子联动逻辑，行为定义如下：

- 全勾选：父节点勾选后，全部子孙节点勾选
- 全取消：父节点取消后，全部子孙节点取消
- 半勾选：部分子节点勾选时，父节点进入半选状态

建议抽取公共工具函数与公共 Hook，避免两端规则漂移。

### 10.2 日志页字段与详情交互

- 耗时列统一读取 `costTime` 字段
- 平台端日志页需与租户端对齐详情能力，至少展示：
  - 请求参数（`params`）
  - 响应结果（`result`）
  - 错误信息（`errorMsg`）
...
前端权限判断与菜单配置中，平台权限节点管理统一使用 `platform:perm:*`，不得再出现 `platform:menu:*`。
