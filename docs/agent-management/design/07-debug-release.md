...
| 对象类型 | 版本号格式 | 示例 | 说明 |
|---------|-----------|------|------|
| Skill | 语义化版本 `MAJOR.MINOR.PATCH` | `1.2.0` | 支持兼容性检查；MAJOR 变更触发引用方确认 |
| Agent | 时间戳格式 `YYYYMMDD-HHmm-SEQ` | `20260318-1430-1` | 每次发布自动生成，不手动指定；同一分钟内多次发布以 SEQ 递增 |
| Workflow | 时间戳格式 `YYYYMMDD-HHmm-SEQ` | `20260318-1430-1` | 同 Agent |
| Knowledge Base | 整数自增 `v1, v2...` | `v3` | 文档集合变更触发版本递增，供引用追溯 |

> 语义化版本（Skill）允许引用方精细控制升级时机；时间戳版本（Agent/Workflow）确保每次发布可审计且有序。两种格式不混用。
...
```json
{
  ...
  "data": {
    "failed_checks": [
      {
        "rule": "prompt_not_empty",
        "description": "System Prompt 不能为空"
      },
      {
        "rule": "model_available",
        "description": "所配置模型 gpt-4o 当前不可用，请切换备用模型"
      }
    ]
  }
}
...
> 前端需遍历 `failed_checks` 数组，逐条展示给用户，不能直接显示 `message` 字段。
...
灰度发布基于**用户 ID 哈希取模**实现流量分流：

```
gray_ratio = 10%  →  hash(user_id) % 100 < 10  →  路由到灰度版本
                      其余用户                  →  路由到稳定版本
```
...
| 参数 | 类型 | 说明 |
|------|------|------|
| `gray_ratio` | integer (1-100) | 灰度流量百分比 |
| `gray_version_id` | string | 灰度版本 ID |
| `stable_version_id` | string | 稳定版本 ID |
| `sticky_session` | boolean | 是否同一用户全程路由到同一版本（默认 true） |

灰度结束后支持两种操作：
- **全量上线**：稳定版本切换为灰度版本，旧稳定版本归档
- **回滚**：灰度版本下线，恢复为稳定版本，灰度版本标记为 ABANDONED

灰度期间，可观测性面板可按版本维度对比关键指标（错误率、响应时延、用户评分）。
...
> **版本物理清理策略**：系统保留最近 20 个已发布版本（每个 Agent/Workflow 独立计算）。超出上限的最旧版本通过定期任务（每日凌晨 2:00）物理删除。以下情况例外：① 当前线上版本（不论排名）永久保留；② 用户手动归档（ARCHIVED 状态）的版本不参与自动清理；③ 已被 A/B 测试引用的版本清理前先解除引用关系。
